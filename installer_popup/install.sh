...
echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}   Claude Code 설치 프로그램${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
install_vscode_extension() {
    ...
    if command -v code &> /dev/null; then
        ...
        # VS Code가 설치되었지만 code 명령어가 PATH에 없는 경우
        VSCODE_PATH="/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code"
        if [ -f "$VSCODE_PATH" ]; then
            "$VSCODE_PATH" --install-extension anthropic.claude-code
            print_status "VS Code Claude 확장 설치 완료"
        else
            echo -e "${YELLOW}!${NC} VS Code를 열고 Command Palette에서 'Shell Command: Install code command' 실행 후"
            echo "   다시 시도해주세요."
        fi
    fi
}
...
main() {
    ...
    echo ""
    echo "다음 단계:"
    echo "  1. VS Code를 실행합니다"
    echo "  2. 왼쪽 사이드바에서 Claude 아이콘을 클릭합니다"
    echo "  3. 로그인을 진행합니다"
    echo ""
    ...
    # VS Code 열기
    read -p "VS Code를 열까요? (Y/n): " open_vscode < /dev/tty
